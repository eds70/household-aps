...
Универсальный перетаскиваемый диалог с изменяемым размером. Используется
**во всех** модальных окнах системы (20 диалогов в 12 страницах).

**Свойства:**

| Проп | Тип | По умолчанию | Описание |
|------|-----|--------------|----------|
| `open` | `bool` | — | Видимость диалога |
| `onClose` | `() => void` | — | Закрытие |
| `title` | `ReactNode` | — | Заголовок (строка или JSX) |
| `titleExtra` | `ReactNode` | — | Доп. элемент в шапке |
| `children` | `ReactNode` | — | Содержимое |
| `actions` | `ReactNode` | — | Кнопки в футере |
| `draggable` | `bool` | `true` | Таскать за заголовок |
| `resizable` | `bool` | `true` | Ресайз за угол |
| `centerOnOpen` | `bool` | `true` | Открывать по центру экрана |
| `initialWidth` | `number \| string` | `700` | Стартовая ширина |
| `minWidth` | `number` | `480` | Минимальная ширина |
| `minHeight` | `number` | `320` | Минимальная высота |
| `showCloseButton` | `bool` | `true` | Крестик в углу |
| `closeOnBackdropClick` | `bool` | `false` | Закрывать по клику вне |

**Особенности:**
- **Esc** — всегда закрывает (стандартное поведение MUI Dialog).
- **Клик вне** — по умолчанию **не** закрывает (защита от случайного
  закрытия формы).
- **Drag** — за заголовок; кнопки, чипы и иконки в шапке не мешают.
- **Resize** — за правый нижний угол; ограничен по экрану (95% ширины,
  90% высоты).
- **Не контролируется** извне по позиции/размеру — состояние локальное,
  сбрасывается при каждом открытии.
