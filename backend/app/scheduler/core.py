...
class ProductionScheduler:
    def __init__(self, horizon_hours: int = 2160):
    ...
    async def build_schedule(self) -> Dict[str, Any]:
        ...
        for eq_id, tasks_list in by_equipment.items():
            ...
            for i in range(k):
                for j in range(i + 1, k):
                    ...
                    i_before_j = model.NewBoolVar(f"eq_{eq_id}_batch_{task_i['batch_id'][:8]}_before_{task_j['batch_id'][:8]}")
        ...
        for cal_event in calendar_minutes:
            ...
            for task_data in affected_tasks:
                ends_before = model.NewBoolVar(f"cal_{task_data['batch_id'][:8]}_{task_data['op_id'][:8]}_before_{start_min}")
...
async def main():
    scheduler = ProductionScheduler(horizon_hours=2160)
