...
async def get_all_operations(
    ...
    for row in result.fetchall():
        operations_list.append({
            ...
            "name": row.name,
            "base_duration_mins": row.base_duration_mins,
            "is_setup": row.is_setup,
            "is_parallel_group": row.is_parallel_group,
            "parallel_group_id": row.parallel_group_id,
            "needs_boiler": row.needs_boiler,
            "needs_cooling_zone": row.needs_cooling_zone,
            "needs_operator": row.needs_operator,
            "needs_lab": row.needs_lab,
...
async def create_operation(
    ...
    """), {
        ...
        "stage_order": operation.stage_order,
        "name": operation.name,
        "base_duration_mins": operation.base_duration_mins,
        "is_setup": operation.is_setup,
        "is_parallel_group": operation.is_parallel_group,
        "parallel_group_id": operation.parallel_group_id,
        "needs_boiler": operation.needs_boiler,
        "needs_cooling_zone": operation.needs_cooling_zone,
        "needs_operator": operation.needs_operator,
        "needs_lab": operation.needs_lab,
        "duration_formula": operation.duration_formula,
...
async def update_operation(
    ...
    query = text(
        f"UPDATE operation_template SET {set_clause} "
        f"WHERE id = :operation_id AND organization_id = :org_id "
        f"RETURNING id, organization_id, product_id, stage_order, name, base_duration_mins, is_setup, is_parallel_group, parallel_group_id, needs_boiler, needs_cooling_zone, needs_operator, needs_lab, duration_formula, comment"
    )
...
async def delete_operation(
    ...
    result = await db.execute(
        text("DELETE FROM operation_template WHERE id = :operation_id AND organization_id = :org_id"),
        {"operation_id": operation_id, "org_id": org_id},
    )
...
async def get_products_for_operations(
        org_id: UUID = Depends(get_current_org_id),
        db: AsyncSession = Depends(get_db_session),
):
    result = await db.execute(
        text("SELECT id, name, code FROM product WHERE organization_id = :org_id AND type = 'PF' ORDER BY name"),
        {"org_id": org_id},
    )
