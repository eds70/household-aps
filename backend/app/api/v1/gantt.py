...
async def get_gantt_data(
    ...
    Если передан version_id, загружает данные из БД.
    Иначе возвращает последний рассчитанный в памяти план.
    """
    if version_id:
        # === ЗАГРУЗКА ИЗ БД (СОХРАНЕННЫЙ ПЛАН) ===
        query = text("""
            SELECT 
                st.id,
                st.batch_id,
                st.planned_start AS start,
                st.planned_end AS end,
        ...
        for row in rows:
            eq_name = str(row.equipment_name or row.equipment_id)[:20] if row.equipment_id else "Unknown"
            prod_name = str(row.product_name or row.product_code or row.product_id)[:20] if row.product_id else "Unknown"
        ...
        # === ЗАГРУЗКА ИЗ ПАМЯТИ (ПОСЛЕДНИЙ РАСЧЕТ) ===
        ...
        for task in tasks:
            ...
            equipment_set.add(eq_name if eq_name else eq_id[:20])
            ...
            gantt_tasks.append(GanttTask(
                ...
                equipment_id=eq_name if eq_name else eq_id[:20],
...
async def export_gantt_to_excel(
    ...
    """Экспорт Ганта в Excel. Поддерживает как последний расчет, так и сохраненную версию."""
    ...
    headers = ["Оборудование", "Операция", "Партия", "Продукт", "Начало", "Конец", "Длительность (мин)"]
